...
def test(args, model, device, test_loader):
    # Note: all test related words here should be replaced as "train"
    model.eval()
    ...
    print('\nTrain set: Average loss: {:.4f}, disp error: {:.4f}, final disp error: {:.4f}\n'.format(
        test_loss, disp_error, fina_disp_error))
    with open(os.path.join(save_log_directory, 'final_train_results_wi_testset_'+str(args.testset)+'.pkl'), 'wb') as f: # format: [(2m X T, 2m X T), (2m X T, 2m X T),...]
...
def main():
    ...
    train_losses = test(saved_args, model, device, train_loader)    # didn't use test_losses for anything for now
